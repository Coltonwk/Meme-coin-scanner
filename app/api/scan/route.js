...
async function loadFeed() {
  const r = await fetch(
    `${feedEndpoint()}?sinceSeconds=1800`,
    {
      cache: "no-store",
      headers: FEED_TOKEN
...
async function dexPairs(mint) {
  const r = await fetch(
    ...
    {
      cache: "no-store",
      headers: { "User-Agent": "meme-coin-social-pulse-v8" }
    }
  );
...
function xHandleFromUrl(url) {
  try {
    ...
    if (!first || ["i", "home", "search", "intent"].includes(first.toLowerCase())) {
      return "";
    }
...
function searchPhrases(coin) {
  ...
  if (symbol && symbol !== "?") {
    phrases.push(`$${symbol}`);
    phrases.push(symbol);
    phrases.push(`${symbol} pump.fun`);
  }

  if (name && name.toLowerCase() !== "new pump.fun launch") {
    phrases.push(name);
    phrases.push(`${name} $${symbol}`);
  }
  ...
  return [...new Set(phrases.filter(Boolean))].slice(0, 14);
}
...
function score(pair) {
  ...
  s += liq >= 25000 ? 30 : liq >= 10000 ? 24 : liq >= 3000 ? 15 : liq >= 1000 ? 7 : 0;
  s += trades >= 25 ? 30 : trades >= 10 ? 22 : trades >= 4 ? 12 : trades >= 2 ? 5 : 0;
  s += vol >= 10000 ? 20 : vol >= 3000 ? 14 : vol >= 500 ? 7 : 0;
  s += ratio >= 2 ? 20 : ratio >= 1.4 ? 14 : ratio >= 1.05 ? 6 : 0;
...
async function enrich(launch) {
  ...
  if (!pairs.length) {
    return {
      ...
      searchHints: {
        twitterHandle: "",
        phrases: [mint, `CA ${mint}`, `contract ${mint}`]
      }
    };
  }
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      version: "8.0-deep-social-search",
      launches
    }, {
